...
async function executeHttpRequest(curlCommand, timeout = 30000) {
    try {
        // 解析curl命令
...
// 解析命令行参数，正确处理引号
function parseCommandLineArgs(command) {
    const args = [];
    let current = '';
    let inQuotes = false;
    let quoteChar = '';
...
async function executeCurl(curlCommand, timeout = 30000) {
    // 首先尝试使用系统curl
    try {
...
export function createTools(consoleManager) {
    return [
        // 1. 创建控制台 - 返回唯一ID
        {
            name: "create_console",
