...
export class ConsoleManager {
    consoles = new Map();
    consolesByName = new Map(); // name -> id mapping
    asyncExecutions = new Map();
     ...
     * 创建新的控制台会话
     */
    async createConsole(shell, workingDir, environment, name, encoding) {
     ...
     */
    resolveConsoleId(consoleIdOrName) {
        // 首先尝试直接作为ID查找
     ...
     */
    getConsoleSession(consoleIdOrName) {
     ...
     * 同步执行命令
     */
    async executeSync(consoleIdOrName, command, timeout = 30000, outputFilter) {
     ...
     * 异步执行命令
     */
    async executeAsync(consoleIdOrName, command, outputFilter) {
        const session = this.getConsoleSession(consoleIdOrName);
     ...
     * 获取异步执行结果
     */
    async getAsyncResult(consoleIdOrName, executionId, outputFilter) {
        const consoleId = this.resolveConsoleId(consoleIdOrName);
        const executions = this.asyncExecutions.get(consoleId);
        if (!executions) {
     ...
     * 获取控制台状态
     */
    getConsoleStatus(consoleIdOrName) {
        const session = this.getConsoleSession(consoleIdOrName);
        return {
            id: session.id,
            name: session.name,
     ...
     * 关闭控制台
     */
    async closeConsole(consoleIdOrName) {
        const session = this.getConsoleSession(consoleIdOrName);
        const consoleId = session.id;
        // 终止进程
     ...
     */
    async cleanup() {
        const promises = Array.from(this.consoles.keys()).map(id => this.closeConsole(id).catch(err => console.error(`Error closing console ${id}:`, err)));
     ...
     * 获取shell初始化参数（包含编码设置）
     */
    getShellInitArgs(shell, encoding) {
        const shellLower = shell.toLowerCase();
        if (shellLower.includes('cmd.exe') || shellLower.includes('cmd')) {
            // CMD: 设置代码页
     ...
     * 添加输出到缓冲区
     */
    addToOutputBuffer(session, output, isError = false) {
     ...
     * 获取控制台输出缓冲区
     */
    async getConsoleOutput(consoleIdOrName, lines, outputFilter) {
        const session = this.getConsoleSession(consoleIdOrName);
        if (!session.isActive) {
            throw new Error(`Console ${consoleIdOrName} is not active`);
        }
     ...
     * 清空控制台输出缓冲区
     */
    async clearConsoleOutput(consoleIdOrName) {
        const session = this.getConsoleSession(consoleIdOrName);
        session.outputBuffer = [];
    }
    applyOutputFilter(stdout, stderr, outputFilter) {
        if (!outputFilter) {
     ...
     * 获取正在运行的执行数量
     */
    getRunningExecutionsCount(consoleId) {
        const executions = this.asyncExecutions.get(consoleId);
        if (!executions) {
            return 0;
        }
